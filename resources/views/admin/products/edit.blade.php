    ...
    <main class="flex-1 p-8">
        <div class="max-w-3xl mx-auto">
            <div class="mb-10">
                ...
                <p class="text-gray-500">Ubah data produk: <strong>{{ $product->name }}</strong></p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
                <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none" required>
                    </div>

                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Harga (Rp)</label>
                            <input type="number" name="price" value="{{ old('price', $product->price) }}" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none" required>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Stok</label>
                            <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none" required>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <label class="block text-sm font-bold text-gray-700 mb-2">Ganti Foto (Kosongkan jika tidak ingin ganti)</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none">{{ old('description', $product->description) }}</textarea>
                    </div>

                    <div class="pt-4 flex gap-4">
                        <button type="submit" class="flex-1 bg-indigo-900 text-white py-3 rounded-xl font-bold hover:bg-indigo-800 shadow-lg transition">Update Produk</button>
